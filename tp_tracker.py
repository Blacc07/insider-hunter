"""
💼 Cloud Take-Profit Tracker (tp_tracker.py)
Runs on GitHub Actions every 2 minutes (scheduled by solana-breakout.yml).

Automatically seeds itself from every new breakout alert stored in the
`alerted_tokens` table, then manages each position with the Quant Rules:

  Rule 1 (TP1):  2x entry            -> alert: SELL 50% (principal recovered)
  Rule 2 (Ladder): 3x / 5x / 10x     -> alert: SELL 10% of INITIAL size each
  Rule 3 (Trailing stop): after TP1, drop >=30% from peak
                                       -> alert: SELL REMAINDER
  Rule 4 (Time decay): no new peak for 2 hours
                                       -> alert: SELL REMAINDER
  Rule 0 (Safety stop): before TP1, drop >=40% from entry
                                       -> alert: SELL EVERYTHING (0.0 disables)

Positions auto-close after a stop / decay / 24h max age.
Network failures are treated as 'skip this cycle', NEVER as a signal.
"""
...
HARD_STOP_DROP = 0.40      # Rule 0: -40% from entry before TP1 (0.0 disables)
...
def seed_new_positions(conn: sqlite3.Connection) -> int:
    """Opens a tracked position for every breakout alert we haven't seen yet."""
    cur = conn.execute(
    ...
    for pair, symbol, token, first_alerted, mcap in cur.fetchall():
        ...
        conn.execute(
            ...
            "entry_mcap, peak_mcap, last_peak_ts, started_ts, tp1, tp2, tp3, tp4, closed, close_reason) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, 0, 0, 0, 0, 0, '')",
            (token, symbol or "UNKNOWN", pair or "", entry, entry, ts, ts),
        )
        n += 1
    conn.commit()
    return n
...
# ---------------------------------------------------------------------------
# 🌐 DEXSCREENER (returns (ok, mcap_map); failures never look like signals)
# ---------------------------------------------------------------------------
...
def evaluate_position(conn: sqlite3.Connection, r, now: datetime, mcap_map: dict) -> int:
    ...
    if entry <= 0:
        ...
        msgs.append(build_message(
            "🕳️", "LIQUIDITY VANISHED", symbol, entry, peak, 0.0,
        ...
        if tp1 == 0 and mult >= TP1_MULT:
            tp1 = 1
            msgs.append(build_message(
                "1️⃣", "TP1 - PRINCIPAL RECOVERED", symbol, entry, peak, current,
                "SELL 50% of position - initial investment is back, the rest is a free ride.", pair))

        # Rule 2: ladder at 3x / 5x / 10x
        if tp1 == 1 and tp2 == 0 and mult >= TP2_MULT:
            tp2 = 1
            msgs.append(build_message(
                "2️⃣", "TP2 - 3x LADDER", symbol, entry, peak, current,
                "SELL 10% of INITIAL size.", pair))
        if tp1 == 1 and tp3 == 0 and mult >= TP3_MULT:
            tp3 = 1
            msgs.append(build_message(
                "3️⃣", "TP3 - 5x LADDER", symbol, entry, peak, current,
                "SELL 10% of INITIAL size.", pair))
        if tp1 == 1 and tp4 == 0 and mult >= TP4_MULT:
            tp4 = 1
            msgs.append(build_message(
                "4️⃣", "TP4 - 10x LADDER", symbol, entry, peak, current,
                "SELL 10% of INITIAL size.", pair))
        ...
        if closed == 0:
            ...
            if flat_h >= TIME_DECAY_HOURS:
                ...
                msgs.append(build_message(
                    "⏳", "TIME DECAY - MOMENTUM DEAD", symbol, entry, peak, current,
        ...
        if closed == 0 and age_h >= MAX_AGE_HOURS:
            ...
            msgs.append(build_message(
                "🏁", "MAX TRACKING AGE (24h)", symbol, entry, peak, current,
                "Tracking stopped - manage any remainder manually.", pair))

    conn.execute(
        "UPDATE tp_state SET peak_mcap=?, last_peak_ts=?, tp1=?, tp2=?, tp3=?, tp4=?, "
        "closed=?, close_reason=? WHERE token_address=?",
        (peak, last_peak_dt.isoformat(), tp1, tp2, tp3, tp4, closed, reason, token),
    )
    conn.commit()
...
# ---------------------------------------------------------------------------
def main() -> None:
    log("💼 Cloud Take-Profit Tracker starting...")
    if not os.path.exists(DB_PATH):
        log("⚠️ DB not found yet (breakout bot has not run). Exiting safely.")
        return
    ...
    rows = conn.execute(
        ...
        "last_peak_ts, started_ts, tp1, tp2, tp3, tp4 "
